<?php consultox_bunch_global_variable();
	...
	$layout = consultox_set( $settings, 'search_page_layout', 'full' );
	$sidebar = consultox_set( $settings, 'search_page_sidebar', 'default-sidebar' );
	...
	$bg = (consultox_set($settings, 'search_page_header_img'));
	$title = consultox_set($settings, 'search_page_header_title');
?>
<!--Page Title-->
        ...
        <div class="row clearfix">
			...
			<?php else : ?>
                <div class="<?php echo esc_attr($classes); ?> blog_post_area eco-search">
                    <p><?php esc_html_e( 'It seems we can&rsquo;t find what you&rsquo;re looking for. Perhaps searching can help.', 'consultox' ); ?></p>
                    <aside class="sidebar">
                    	<?php get_search_form(); ?>
                    </aside>
                </div>
